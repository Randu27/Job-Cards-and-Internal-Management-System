...
(function initAddPage() {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (hasAccess) {
      ...
      if (!lEm || !lPw) { showPopup("error", "Website Access Incomplete", "Please fill in Login Email and Password."); return; }
    ...
    try {
      ...
      if (hasAccess) {
        const lEm = document.getElementById("loginEmail").value;
        const lPw = document.getElementById("loginPassword").value;
        const grantedPages = window.getSelectedPages ? window.getSelectedPages() : [];
        try {
          const cred = await firebase.auth().createUserWithEmailAndPassword(lEm, lPw);
          data.accessDetails = { uid: cred.user.uid, email: lEm, grantedPages };
        } catch (authErr) {
          if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = origHTML; }
...
(function initDirectoryPage() {
  ...
  function populateViewMode(emp) {
    ...
    if (emp.websiteAccess && emp.accessDetails) {
      accessVal = `Enabled — Login: ${emp.accessDetails.email} | Pages: ${(emp.accessDetails.grantedPages || []).join(', ') || 'None'}`;
    }
    ...
    try {
      ...
      if (wantAccess) {
        if (!alreadyHadAccess) {
          ...
          const grantedPages = existingEmp?.accessDetails?.grantedPages || [];
          accessDetails = { uid: cred.user.uid, email: editLoginEm, grantedPages };
        } else {
          const grantedPages = existingEmp?.accessDetails?.grantedPages || [];
          accessDetails = { ...existingEmp.accessDetails, email: editLoginEm, grantedPages };
        }
      } else {
        accessDetails = null;
        websiteAccess = false;
